...
def test_the_token_goes_only_on_ray_s_two_verbs(qapp, pump):
    ...
    c = PathClient("http://path", get=get, request=request)
    c.set_token("  " + TOKEN + "  ")
...
def test_the_token_is_never_written_anywhere(qapp, tmp_path, caplog):
    ...
    assert not any(TOKEN in p.read_text(errors="ignore") for p in tmp_path.rglob("*")
                   if p.is_file())
    panel.stop_clients()
    assert c.token == ""                                             # gone at quit
