...
from ade_desktop.geometry import Rect, clamp_to_screens, load_state, save_state
...
class OrbController(QObject):
    def __init__(self, *, window, orb, renderer, mood, speaker, mic, voice,
        ...
        self.mic.utterance.connect(self.voice.on_utterance)
    ...
    def tick(self) -> None:
        ...
        now = self._clock()
        look = self.look(now)
        dpr = self.orb.devicePixelRatioF() or 1.0
        ...
        want = LIVELY_MS if lively else RESTING_MS
    ...
    def _on_size(self, act) -> None:
        ...
        if key in SIZES and key != self.size_key:
            ...
            self.orb.move(centre.x() - px // 2, centre.y() - px // 2)
            self.save()
